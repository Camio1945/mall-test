...
public class OmsPortalOrderServiceImpl implements OmsPortalOrderService {
  ...
  @Autowired private UmsMemberReceiveAddressService memberReceiveAddressService;
  ...
  public Map<String, Object> generateOrder(OrderParam orderParam) {
    ...
    UmsMemberReceiveAddress address =
        memberReceiveAddressService.getItem(orderParam.getMemberReceiveAddressId());
    ...
    if (orderParam.getUseIntegration() != null) {
      ...
      memberService.updateIntegration(
          currentMember.getId(), currentMember.getIntegration() - orderParam.getUseIntegration());
    }
    // 删除购物车中的下单商品
   ...
   */
  private void handleCouponAmount(
      List<OmsOrderItem> orderItemList, SmsCouponHistoryDetail couponHistoryDetail) {
  ...
  private BigDecimal getUseIntegrationAmount(
      Integer useIntegration, BigDecimal totalAmount, UmsMember currentMember, boolean hasCoupon) {
  ...
  /**
   * 删除下单商品的购物车信息<br>
   * 【性能优化备注】删除购物车信息在线程池中异步执行（万一失败了，也不影响已经提交的订单）
   *
   * @param cartPromotionItemList
   * @param currentMember
   */
  private void deleteCartItemList(
      List<CartPromotionItem> cartPromotionItemList, UmsMember currentMember) {
    ...
    System.out.println(
        "com.macro.mall.portal.service.impl.OmsPortalOrderServiceImpl.deleteCartItemList Time elapsed : "
  ...
  public Integer cancelTimeOutOrder() {
    ...
    OmsOrderSetting orderSetting = orderSettingMapper.selectByPrimaryKey(1L);
    // 查询超时、未支付的订单及订单详情
    List<OmsOrderDetail> timeOutOrders =
        portalOrderDao.getTimeOutOrders(orderSetting.getNormalOrderOvertime());
    ...
    for (OmsOrderDetail timeOutOrder : timeOutOrders) {
      ...
      if (timeOutOrder.getUseIntegration() != null) {
        ...
        memberService.updateIntegration(
            timeOutOrder.getMemberId(), member.getIntegration() + timeOutOrder.getUseIntegration());
      }
    }
  ...
  private List<OmsOrderItem> getCouponOrderItemByRelation(
      SmsCouponHistoryDetail couponHistoryDetail, List<OmsOrderItem> orderItemList, int type) {
